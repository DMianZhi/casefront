// M1 去噪：同类元素折叠（精简模式核心）
// 两级折叠策略（同一父节点 + 同角色，兄弟数 ≥ minSiblings 才启用）：
//   1) 公共前缀 ≥ 2 字符 → 折叠（「复制 路径 A~E」→「复制 路径 #」）
//   2) 前缀不成，尾 token 全相同 → 折叠（「乌鸦 0」「35人英雄… 0」→「# 0」等价类）
// 存疑不删：成员收缩为 representative，memberCount 记录规模，明细可由 Skill 侧展开
// mode=full 时跳过折叠（完整模式，配合去噪阈值可配）
...
function makeGroup(kind, signature, members) {
  return {
    kind,                    // 'prefix' | 'suffix'
    signature,               // 前缀文本 或 尾 token
    role: members[0].role,
...
export function foldGroups(nodes, { mode = 'compact', minSiblings = 5 } = {}) {
